...
class Authentication {
    ...
    authorize(credentials) {
        console.log('Checking' + credentials.installed.client_id);
        ...
        return new Promise((resolve, reject)=>{
            ...
            if(process.env.HEROKU) {
                ...
                console.log('getting token from heroku' + process.env.TOKEN + '  --  ' + token);
                    if (!token || token === 0) {
                ...
                fs.readFile(TOKEN_PATH, (err, token) => {
                    console.log('getting token from local' + token);
                    if (err) {
                        this.getNewToken(oauth2Client).then((oauth2ClientNew)=>{
                            resolve(oauth2ClientNew);
                        }, (err)=>{
